...
\subsection{Krigeage} La dernière partie de notre projet s'oriente autour de la methode dite krigeage, qui est effectivement une union de la simulation spatiale
avec le problème numérique des moindres carrés. Il s'agit d'interpoler à partir d'un ensemble des observations $Z(s)$ pour un certain nombre de sites $s \in \mathcal{D}$. Par exemple, on pourrait
prendre $Z(s)$ la concentration chimique sur la surface d'une région rectangulaire. Comme il est impossible d'observer la concentration à chaque site $s \in \mathbb{R}^2$,
on se contente d'observer la concentration en un nombre finie de site et en déduire les valeurs pour les autres sites dont la concentration est inconnue.


La formulation mathématique est la suivante :On commence avec un ensemble de $n$ sites $\{s_1, \dots, s_n\}$ et leurs mesures associés $Z(s_i)$, et estimer
$\hat Z(s)$ pour tout $s \in \mathcal{D}$. La théorie consiste a construire un estimateur $\hat Z$ qui est exacte, c'est-à-dire que $\hat Z(s_i) = Z(s_i)$ pour
...
Il est intéressant à remarquer que pour chaque $s \in \mathcal{D}$ dont on aimerait estimer $\hat Z(s)$, le système d'équations à résoudre ne dépend
...
$\hat Z(s)$. Sans tarder, étudions un exemple avec $\mathcal{D} = \mathbb{R}^2$.

\subsubsection{Réalisation du krigeage}

Vu qu'on est pas munis des mesures avec lesquelles on pourrait interpoler, on va utiliser la méthode décrite dans la dernière section pour simuler un champ gaussien
...
Après la réalisation d'un champ gaussien, on va calculer $\hat Z(s)$ pour toute site de notre grille, en résolvant un système d'équations \textbf{unique} pour chaque $s \in \mathcal{D}$, après
...
Ce qui est important à retenir est que les illustrations à droites ne sont pas censées réproduire le champ à gauche avec lequel
on est partir. Ce que le krigeage fait , comme l'interpolation polynômiale, c'est de toucher les bonnes valeurs où les mesures ont été observés puis ensuite
...
un ensemble de mesures de profondeurs d'un câble sous-marin, on peut effectivement interpoler la profondeur entre les sites de mesures afin de calculer sa longueur.
