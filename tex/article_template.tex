...
tout élément $h$ de $H$ afin de trouver $\Sigma$, un comportement qui est mis en œuvre par la fonction \texttt{get\_cov\_matrix_exp(n, a)}
pour une grille de $n$ lignes et $n$ colonnes. Dans notre exemple, nous appelons la fonction \texttt{get\_cov\_matrix_exp(3, 1)}.
...
ligne et $n^2$ colonne pour une grille $n$ fois $n$le nombre d'opérations droites pour calculer $\Sigma$est de complexité $\mate cal{O}(n^4)$ .
Et enfin, la décomposition de Cholesky est de complexité $\mate cal{O}(n^3)$ ainsi que la multiplication de la matrice.
